...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "set-chat-model") {
      if (callerEmail !== ROOT_ADMIN_EMAIL) {
        return json({ error: "Root admin access required." }, 403);
      }

      const option = CHAT_MODEL_OPTIONS.find((modelOption) => modelOption.id === body?.modelId);
      if (!option) return json({ error: "Unknown model option." }, 400);
      ...
      return json(await getAdminPanelData(admin, callerEmail));
    }
...
async function getAdminPanelData(admin: any, callerEmail: string) {
  const adminEmails = await getAdminEmails(admin);
  const users = await listUsers(admin);
  const rootAdmin = callerEmail === ROOT_ADMIN_EMAIL;

  return {
    rootAdminEmail: ROOT_ADMIN_EMAIL,
    chatModelOptions: rootAdmin ? CHAT_MODEL_OPTIONS : [],
    selectedChatModel: rootAdmin ? await getSelectedChatModel(admin) : null,
...
async function getSelectedChatModel(admin: any) {
  ...
  const option = CHAT_MODEL_OPTIONS.find(
    (modelOption) => modelOption.provider === value.provider && modelOption.model === value.model
  );

  return option ?? CHAT_MODEL_OPTIONS[0];
}
